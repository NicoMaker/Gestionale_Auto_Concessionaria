...
router.post("/", (req, res) => {
  ...
  db.run(
    sql,
    [nome, cognome, email || null, telefono || null, indirizzo || null],
    function (err) {
      if (err) return res.status(500).json({ error: err.message });
      db.get("SELECT * FROM clienti WHERE id = ?", [this.lastID], (e, row) =>
...
router.put("/:id", (req, res) => {
  ...
  db.run(
    ...
    function (err) {
      ...
      db.get("SELECT * FROM clienti WHERE id = ?", [req.params.id], (e, row) =>
        res.json(row),
      );
    },
  );
