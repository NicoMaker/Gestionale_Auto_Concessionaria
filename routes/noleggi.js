...
router.post("/", (req, res) => {
  ...
  if (!auto_id || !cliente_id || !data_inizio) {
    return res
      .status(400)
      .json({ error: "Auto, cliente e data inizio sono obbligatori" });
  }
  ...
  db.run(
    ...
    function (err) {
      ...
      db.get(SELECT_BASE + " WHERE noleggi.id = ?", [this.lastID], (e, row) =>
...
router.put("/:id", (req, res) => {
  ...
  db.run(
    ...
    function (err) {
      if (err) return res.status(500).json({ error: err.message });
      if (this.changes === 0)
        return res.status(404).json({ error: "Noleggio non trovato" });
      if (stato === "concluso") {
        db.run("UPDATE auto SET stato = ? WHERE id = ?", [
          "disponibile",
          auto_id,
        ]);
      }
      db.get(SELECT_BASE + " WHERE noleggi.id = ?", [req.params.id], (e, row) =>
        res.json(row),
      );
    },
  );
