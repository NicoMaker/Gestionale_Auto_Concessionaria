...
router.get('/stats/riepilogo', (req, res) => {
  ...
  `, (err, row) => {
    ...
    db.get('SELECT COUNT(*) AS totale_clienti FROM clienti', (err2, row2) => {
      ...
      db.get(`SELECT COALESCE(SUM(costo),0) AS totale_costi FROM manutenzioni`, (err3, row3) => {
...
// POST nuova auto
router.post('/', (req, res) => {
  const { targa, marca, modello, anno, colore, km, prezzo, stato, cliente_id } = req.body;
  if (!targa || !marca || !modello) {
    return res.status(400).json({ error: 'Targa, marca e modello sono obbligatori' });
  }
  const sql = `INSERT INTO auto (targa, marca, modello, anno, colore, km, prezzo, stato, cliente_id)
               VALUES (?,?,?,?,?,?,?,?,?)`;
  db.run(sql, [targa, marca, modello, anno || null, colore || null, km || 0, prezzo || 0, stato || 'disponibile', cliente_id || null], function (err) {
    if (err) {
      if (err.message.includes('UNIQUE')) return res.status(409).json({ error: 'Targa gia\' esistente' });
      return res.status(500).json({ error: err.message });
    }
    db.get(SELECT_BASE + ' WHERE auto.id = ?', [this.lastID], (e, row) => res.status(201).json(row));
  });
});
...
router.put('/:id', (req, res) => {
  ...
  const sql = `UPDATE auto SET targa=?, marca=?, modello=?, anno=?, colore=?, km=?, prezzo=?, stato=?, cliente_id=?
               WHERE id=?`;
  db.run(sql, [targa, marca, modello, anno || null, colore || null, km || 0, prezzo || 0, stato, cliente_id || null, req.params.id], function (err) {
    if (err) return res.status(500).json({ error: err.message });
    if (this.changes === 0) return res.status(404).json({ error: 'Auto non trovata' });
    db.get(SELECT_BASE + ' WHERE auto.id = ?', [req.params.id], (e, row) => res.json(row));
  });
});
