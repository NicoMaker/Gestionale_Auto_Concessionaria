...
router.post('/', (req, res) => {
  ...
  if (!auto_id || !data || !tipo) {
    return res.status(400).json({ error: 'Auto, data e tipo sono obbligatori' });
  }
  const sql = `INSERT INTO manutenzioni (auto_id, data, tipo, descrizione, costo, km) VALUES (?,?,?,?,?,?)`;
  db.run(sql, [auto_id, data, tipo, descrizione || null, costo || 0, km || null], function (err) {
    if (err) return res.status(500).json({ error: err.message });
    db.get(SELECT_BASE + ' WHERE manutenzioni.id = ?', [this.lastID], (e, row) => res.status(201).json(row));
  });
});
...
router.put('/:id', (req, res) => {
  ...
  db.run(sql, [auto_id, data, tipo, descrizione || null, costo || 0, km || null, req.params.id], function (err) {
    if (err) return res.status(500).json({ error: err.message });
    if (this.changes === 0) return res.status(404).json({ error: 'Manutenzione non trovata' });
    db.get(SELECT_BASE + ' WHERE manutenzioni.id = ?', [req.params.id], (e, row) => res.json(row));
  });
});

// DELETE manutenzione
router.delete('/:id', (req, res) => {
  db.run('DELETE FROM manutenzioni WHERE id = ?', [req.params.id], function (err) {
    if (err) return res.status(500).json({ error: err.message });
    if (this.changes === 0) return res.status(404).json({ error: 'Manutenzione non trovata' });
