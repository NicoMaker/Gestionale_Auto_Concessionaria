...
export async function renderNoleggiPage(container) {
        ...
        columns: [
          {
            ...
            label: "Auto",
            render: (r) =>
              `${r.marca} ${r.modello} <span class="text-dim">(${r.targa})</span>`,
          },
          {
            key: "cliente",
            label: "Cliente",
            render: (r) => `${r.cliente_nome} ${r.cliente_cognome}`,
          },
          { key: "data_inizio", label: "Inizio" },
          {
            key: "data_fine",
            label: "Fine",
            render: (r) => r.data_fine || '<span class="text-dim">—</span>',
          },
          {
            key: "prezzo_giornaliero",
            label: "€ / giorno",
            render: (r) => formatCurrency(r.prezzo_giornaliero),
          },
  ...
  function openForm(existing) {
    if (autoList.length === 0 || clienti.length === 0) {
      showToast(
        "Servono almeno un'auto e un cliente per creare un noleggio",
        "error",
      );
      return;
    }
      ...
      fields: [
        {
          ...
          type: "select",
          required: true,
          value: existing?.auto_id,
          options: autoList.map((a) => ({
            value: a.id,
            label: `${a.marca} ${a.modello} (${a.targa})`,
          })),
        },
        {
          name: "cliente_id",
        ...
        },
        {
          ...
          type: "date",
          required: true,
          value: existing?.data_inizio || todayISO(),
        },
        {
          name: "data_fine",
          label: "Data fine (opzionale)",
          type: "date",
          value: existing?.data_fine,
        },
        {
          ...
          type: "number",
          step: "0.01",
          value: existing?.prezzo_giornaliero ?? 0,
        },
        {
          name: "stato",
          label: "Stato",
          type: "select",
          value: existing?.stato || "in corso",
          options: STATI.map((s) => ({
            value: s,
