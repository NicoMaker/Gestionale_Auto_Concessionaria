...
export async function renderDashboard(container) {
  mountTopbar({ title: 'Dashboard', subtitle: 'Panoramica generale del gestionale' });
  ...
  const cards = [
    { label: 'Auto totali', value: stats.totale ?? 0, icon: '🚗', color: 'info' },
    { label: 'Disponibili', value: stats.disponibili ?? 0, icon: '✅', color: 'success' },
    { label: 'Noleggiate', value: stats.noleggiate ?? 0, icon: '📅', color: 'purple' },
    { label: 'In manutenzione', value: stats.in_manutenzione ?? 0, icon: '🔧', color: 'warning' },
    { label: 'Clienti registrati', value: stats.totale_clienti ?? 0, icon: '👥', color: 'info' },
    { label: 'Valore parco auto', value: formatCurrency(stats.valore_totale), icon: '💰', color: 'success' }
  ];
  ...
  const table = renderTable({
    ...
    rows: recent,
    emptyText: 'Nessuna auto presente. Aggiungine una dalla sezione "Parco Auto".'
  });
